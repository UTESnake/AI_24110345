...
def greedy(grid, start, goal):
    ...
    while frontier:
        n = min(frontier, key=lambda node: problem.h(node.state))

        if problem.is_goal(n.state):
            return extract_path(n)

        frontier.remove(n)
        frontier_states.discard(n.state)
        reached.add(n.state)

        for child in problem.expand(n):
            m_state = child.state
...
def a_star(grid, start, goal):
    problem = GridProblem(grid, start, goal)
    counter = 0 # Dùng làm tie-breaker cho heapq
    start_node = Node(state=start, parent=None)
    
    # Dictionary lưu chi phí g(n) thực tế tốt nhất hiện tại để đến một node
    g_costs = {start: 0}
    
    # Tính f(Start) = g(Start) + h(Start) = 0 + h(Start)[cite: 1]
    ...
    while frontier:
        ...
        if g_costs[node.state] < f - problem.h(node.state):
            continue
        ...
        for child in problem.expand(node):
            ...
            if m_state not in g_costs or new_g < g_costs[m_state]:
...
# Các hàm bổ trợ cho các thuật toán trên
class Node:
...
# ── Problem class dùng cho IDS và Greedy ─────────────────────────────────────
class GridProblem:
